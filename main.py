...
initial_extensions = [
                      ...
                      'buttons.commissions.commissioncomplete',
                      'buttons.freelancer.freelancersystem',
                      'buttons.freelancer.referrals',
                      ...
                      'src.freelancer.referrals',
                      'src.freelancer.reviews',
...
@client.command()
@commands.is_owner()
async def delete10(ctx):
    db = await aiosqlite.connect('database.db')
    await db.execute('DROP TABLE close;')
    await db.commit()
    await db.close()
    a = await ctx.reply('Done!')
    await asyncio.sleep(5)
...
@client.command()
@commands.is_owner()
async def sqlite13(ctx):
    db = await aiosqlite.connect('database.db')
    cursor = await db.execute("""
   CREATE TABLE storedreferrals (
        creator_id INTEGER,
        member_id INTEGER
    )""")
    await cursor.close()
...
@client.command()
@commands.is_owner()
async def delete13(ctx):
    db = await aiosqlite.connect('database.db')
    await db.execute('DROP TABLE storedreferrals;')
    await db.commit()
    await db.close()
    a = await ctx.reply('Done!')
    await asyncio.sleep(5)
