...
class ReferralsCog(commands.GroupCog, name="referrals"):
            ...
            for r in a:
                if r[1] == i.code:
                    if r[2] != i.uses:
                        ...
                        embed = discord.Embed(title="Referral Added", description=f"{member.mention} has been joined using your referral link!", color=discord.Color.from_str(embed_color))
    ...
    async def generate(self, interaction: discord.Interaction) -> None:
        ...
        if a is None:
            ...
            embed = discord.Embed(description=f"You have successfully created a refferal link! The referral link is `{invite.code}` ({invite})!", color=discord.Color.from_str(embed_color))
            await interaction.response.send_message(embed=embed, ephemeral=True)
        else:
            try:
                ...
                invite = await interaction.channel.create_invite()
                await db.execute('INSERT INTO referrals VALUES (?,?,?,?);', (interaction.user.id, invite.code, 0, 0))
                embed = discord.Embed(description=f"Your previous referral link was invalid. The new valid referral link is `{invite.code}` ({invite})!", color=discord.Color.from_str(embed_color))
                await interaction.response.send_message(embed=embed, ephemeral=True)
        await db.commit()
        await db.close()
