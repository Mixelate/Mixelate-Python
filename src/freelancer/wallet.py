...
class WalletCog(commands.GroupCog, name="wallet"):
    ...
    @app_commands.describe(user="Who would you like to give funds to?")
    @app_commands.describe(amount="How much would you like to give them?")
    ...
    @app_commands.describe(amount="What is your desired amount?")
...
class WalletWalletCog(commands.Cog):
    ...
    async def walletwallet(self, interaction: discord.Interaction) -> None:
        embed = discord.Embed(
            color=discord.Color.from_str(embed_color))
        embed.set_author(name=f"{interaction.user.name}'s Wallet", icon_url=interaction.user.display_avatar.url)
