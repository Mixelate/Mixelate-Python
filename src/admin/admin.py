...
class AdminCog(commands.GroupCog, name="admin"):
    ...
    async def resetwallet(self, interaction: discord.Interaction) -> None:
        ...
        await db.commit()
        await db.close()
        embed = discord.Embed(description="Mixelate's wallet has been reset!", color=discord.Color.from_str(embed_color))
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def strip(self, interaction: discord.Interaction, member: discord.Member) -> None:
        ...
        try:
            ...
            embed = discord.Embed(description="I do not have permission to remove the roles. Please update my permissions!", color=discord.Color.red())
