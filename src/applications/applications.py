...
class ApplicationManager2(discord.ui.View):
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
            ...
            a = await cursor.fetchone()
            if a is None:
                await interaction.response.send_message("This application has already been accepted or denied! Please get them to create a new application if they wish to apply again!", ephemeral=True)
            else:
                ids = []
                x = a[2].split(", ")
                for roles in x:
...
class ApplicationsCog(commands.Cog):
    ...
    async def application(self, interaction: discord.Interaction):
        ...
        a = await cursor.fetchone()
        if a is None:
            await interaction.response.send_message("This channel is either not an application channel or it has already been accepted/denied.", ephemeral=True)
        else:
            view = ApplicationManager2()
            embed = discord.Embed(
                description="Do you accept or deny this application?", 
                color=discord.Color.from_str(embed_color))
