...
paypal_business_name = data["PayPal"]["PAYPAL_BUSINESS_NAME"]
...
async def createinvoice(total, channel_id):
    invoice = Invoice({
        "merchant_info": {
            "business_name": paypal_business_name,
        },
        "items": [
            {
                "name": "Commission",
        ...
        "note": f"An order processed in the {paypal_business_name} Discord server. ({channel_id})",
        "payment_term": {
            "term_type": "NET_45"
        }
    }, api=my_api)
