...
class TicketsCog(commands.Cog):
    ...
    @app_commands.command(name="addaccess", description="Adds someone to the ticket!")
    @app_commands.describe(user="Who would you like to add to the ticket?")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def addaccess(self, interaction: discord.Interaction, user: discord.Member) -> None:
    ...
    @app_commands.command(name="removeaccess", description="Removes someone from the ticket!")
    @app_commands.describe(user="Who would you like to remove from the ticket?")
    @app_commands.guilds(discord.Object(id=guild_id))
    async def removeaccess(self, interaction: discord.Interaction, user: discord.Member) -> None:
        valid_names = ("order", "apply", "support")
        if any(thing in interaction.channel.name for thing in valid_names):
            await interaction.channel.set_permissions(user,
                                                send_messages=False,
